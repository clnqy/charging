...
const emptyFilters = {
  orgDept: '',
  lineNo: '',
  ...Object.fromEntries(filterFields.map((item) => [item.key, ''])),
}
...
const VehicleBase = () => {
  ...
  const [orgDeptOpen, setOrgDeptOpen] = useState(false)
  ...
  const orgDeptTree = useMemo(() => {
    return vehicles.reduce((tree, item) => {
      if (!item.orgName) return tree
      if (!tree[item.orgName]) tree[item.orgName] = new Set()
      if (item.deptName) tree[item.orgName].add(item.deptName)
      return tree
    }, {})
  }, [vehicles])

  const selectedOrgDeptLabel = useMemo(() => {
    if (!filters.orgDept) return '组织 / 部门'
    const [orgName, deptName] = filters.orgDept.split('::')
  ...
  const filteredStationOptions = useMemo(() => {
    ...
    return stationOptions.filter((name) => !keyword || name.toLowerCase().includes(keyword))
  }, [stationKeyword])
  ...
  const filteredVehicles = useMemo(() => {
    ...
    const lineKeyword = filters.lineNo.trim().toLowerCase()
    return vehicles.filter((item) => {
      ...
      const [selectedOrg, selectedDept] = filters.orgDept ? filters.orgDept.split('::') : ['', '']
      const matchOrgDept = !filters.orgDept || (item.orgName === selectedOrg && (!selectedDept || item.deptName === selectedDept))
      const matchLine = !lineKeyword || String(item.lineNo || '').toLowerCase().includes(lineKeyword)
      const matchFilters = filterFields.every((field) => !filters[field.key] || item[field.key] === filters[field.key])
      return matchKeyword && matchOrgDept && matchLine && matchFilters
    })
  ...
  const openForm = (row) => {
    ...
    setStationKeyword(row.stationName || '')
  ...
  const fieldControl = (col) => {
    const disabled = col.key !== 'stationName'

    if (col.key === 'stationName') {
      return (
        <div key={col.key} className="relative" data-station-selector>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setStationKeyword(value)
              setStationDropdownOpen(true)
              setForm((prev) => ({ ...prev, stationName: value }))
            }}
            placeholder="请选择站点"
            className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:border-primary ${errors[col.key] ? 'border-red-400 bg-red-50' : 'border-gray-200'}`}
          />
          {stationDropdownOpen && (
            <div className="absolute z-30 mt-1 w-full bg-white border border-gray-200 rounded shadow-lg max-h-56 overflow-auto">
              {filteredStationOptions.length > 0 ? (
                filteredStationOptions.map((name) => (
                  <button
                    key={name}
                    type="button"
                    onMouseDown={(e) => e.preventDefault()}
                    onClick={() => {
                      setStationKeyword(name)
                      setForm((prev) => ({ ...prev, stationName: name }))
                      setStationDropdownOpen(false)
                    }}
                ...
                <div className="px-3 py-2 text-sm text-gray-400">未找到匹配站点</div>
              )}
            </div>
          )}
    ...
    return (
      <div key={col.key}>
        ...
        <input
          ...
          className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:border-primary ${errors[col.key] ? 'border-red-400 bg-red-50' : 'border-gray-200'} ${disabled ? 'bg-gray-100 text-gray-500' : ''}`}
        />
        {errors[col.key] && <p className="text-xs text-red-500 mt-1">{errors[col.key]}</p>}
      </div>
    )
    ...
    <div className="page-container min-w-0 overflow-hidden">
      <div className="bg-white rounded-lg shadow-sm p-3 mb-3 flex flex-col gap-2 min-h-0" style={{ height: '15%' }}>
        <div className="flex items-center gap-2 min-h-0" style={{ height: '58%' }}>
          ...
          <div className="grid grid-cols-4 gap-2 min-w-0" style={{ width: '56%' }}>
            <div className="relative min-w-0">
              <button
                type="button"
                onClick={() => setOrgDeptOpen((open) => !open)}
                className={`w-full px-2 py-2 border border-gray-200 rounded text-sm bg-white focus:outline-none focus:border-primary min-w-0 text-left truncate ${filters.orgDept ? 'text-gray-700' : 'text-gray-400'}`}
                title={selectedOrgDeptLabel}
              >
                {selectedOrgDeptLabel}
              </button>
              {orgDeptOpen && (
                <div className="absolute z-30 mt-1 w-72 max-h-72 overflow-auto bg-white border border-gray-200 rounded shadow-lg p-2">
                  <button
                    type="button"
                    onClick={() => {
                      setFilters((prev) => ({ ...prev, orgDept: '' }))
                      setOrgDeptOpen(false)
                      resetPage()
                    }}
                    className="w-full text-left px-2 py-1.5 text-sm text-gray-500 hover:bg-blue-50 rounded"
                  >
                    全部组织 / 部门
                  </button>
                  {Object.entries(orgDeptTree).map(([orgName, deptSet]) => (
                    <div key={orgName} className="mt-1">
                      <button
                        type="button"
                        onClick={() => {
                          setFilters((prev) => ({ ...prev, orgDept: `${orgName}::` }))
                          setOrgDeptOpen(false)
                          resetPage()
                        }}
                        className="w-full text-left px-2 py-1.5 text-sm font-medium text-gray-700 hover:bg-blue-50 rounded"
                      >
                        {orgName}
                      </button>
                      {[...deptSet].map((deptName) => (
                        <button
                          key={`${orgName}-${deptName}`}
                          type="button"
                          onClick={() => {
                            setFilters((prev) => ({ ...prev, orgDept: `${orgName}::${deptName}` }))
                            setOrgDeptOpen(false)
                            resetPage()
                          }}
                          className="w-full text-left pl-6 pr-2 py-1.5 text-sm text-gray-600 hover:bg-blue-50 rounded"
                        >
                          {deptName}
                        </button>
                      ))}
            ...
            <input
              ...
              onChange={(e) => {
                setFilters((prev) => ({ ...prev, lineNo: e.target.value }))
                resetPage()
              }}
              placeholder="线路号"
              className="px-2 py-2 border border-gray-200 rounded text-sm bg-white focus:outline-none focus:border-primary min-w-0"
            />
          ...
          <div className="flex items-center justify-end gap-2" style={{ width: '16%' }}>
            ...
            <button onClick={() => { setFilters(emptyFilters); setQuery(''); setOrgDeptOpen(false); resetPage() }} className="btn-secondary text-sm">重置</button>
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 min-h-0" style={{ height: '42%' }}>
          <button onClick={() => setModal('import')} className="btn-secondary text-sm flex items-center gap-1"><Upload className="w-4 h-4" />导入</button>
      ...
      <Modal isOpen={modal === 'edit'} onClose={() => setModal(null)} title={`编辑车辆 - ${form.plateNo || ''}`} onConfirm={saveForm} confirmText="保存">
        <div className="space-y-4" onClick={(e) => {
          if (!e.target.closest('[data-station-selector]')) setStationDropdownOpen(false)
        }}>
